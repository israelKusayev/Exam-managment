    ...
    authManager.getAdminByEmail(email, data => {
      if (!data || data.error) {
        ...
        if (
          data &&
          data[0] &&
          bcrypt.compareSync(password, data[0].PasswordHash)
        ) {
          const dbUser = data[0];
          const user = {
            email: dbUser.Email,
      ...
      if (data && data[0].Success) {
        if (data[0].RowsCount != 0) {
          res
            ...
            .send(
              activationHTML(
                'Exams system - Activation Success',
...
router.post('/admin-getemail-by-resetpassword-token', (req, res) => {
  ...
  if (!rawToken) res.status(400).send({ message: 'No token provided' });
  else {
    try {
...
router.post('/admin-is-token-valid', (req, res) => {
  const rawToken = req.body.token;
  if (!rawToken) res.status(200).send(false);
  else {
    try {
